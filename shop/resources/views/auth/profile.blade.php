...
<body>
	...
	<div class="home" style="padding-top: 20px;height: 50px;">
			...
			<div class="row">
				...
				<div class="col-lg-8 contact_col">
					<div class="get_in_touch">
						<div class="section_title">Ваши данные</div>
						<div class="contact_form_container">
							<form action="{{ route('changeInfo_process')}}" method="POST" id="contact_form" class="contact_form">
								@csrf
								<div class="row">
									<div class="col-xl-6">
										<label for="name">Ваше имя:</label>
										<input type="text" id="name" name="name" class="contact_input" placeholder="Имя" value="{{$User->name ?? old('name')}}">
										@error('login')
								...
								<div>
									...
									<input type="text" id="email" name="email" class="contact_input" placeholder="Почта" value="{{$User->email ?? old('email')}}">
									 @error('login')
									<div class="alert alert-danger">{{$message}}</div>
									@enderror
								</div>
								<div>
									...
									<input type="text" id="phone" name="phone" class="contact_input" placeholder="Номер телефона" value="{{$User->phone ?? old('phone')}}">
									 @error('phone')
									<div class="alert alert-danger">{{$message}}</div>
									@enderror
								</div>
								<div>
									<label for="password">Изменить текущий пароль:</label>
									<input type="password" id="password" name="password" class="contact_input" placeholder="Для того чтобы сохранить новые данные введите текущий или новый пароль!">
									@error('login')
									<div class="alert alert-danger">{{$message}}</div>
									@enderror
								</div>
								<button class="button contact_button" type="submit" name="sendMe"><span>Сохранить</span></button>
							</form>
						</div>
					</div>
				</div>
